...
excerpt: "Like everybody else, I have a lot going on in my life. There is working full-time at Edenspiekermann, focusing on the relationship with my girlfriend, keeping the flat a non-chaotic place, seeing friends, petting the cats, enjoying the city, working on open-source projects, and much more. Handling everything at once can be hard and overwhelming."
post_date: "November 24, 2015"
post_number: 1
tags: 
  - trello
  - workflow
...
<code>Button--justALittleSmaller</code>

<pre>
  <code>
    $main-color: #C5003E;

    @mixin underLine($color: $main-color, $underlineThickness: 1px) {
      color: $color;
      background: linear-gradient(90deg, $color 66%, transparent 0) repeat-x;
      background-size: .2em $underlineThickness;
      background-position: 0 1.3em;
    }
        
    @mixin dashedUnderLine($color: $main-color, $underlineThickness: 1px) {
      color: $color;
    ...
    blockquote a {
     ...
     &:hover {
       @include dashedUnderLine($main-color, 2px);
     }
    }
  </code>
</pre>

I have to admit it took me <code>Button--justALittleSmaller</code>a few days to get used to it and to actually think of <code>Button--justALittleSmaller</code>launching <code>Button--justALittleSmaller</code>Trello and adding cards to the board <code>Button--justALittleSmaller</code>when needed. I also set up the Trello app on my phone so it’s easy to create a card no matter where I am or if I have connectivy (as the app works seamlessly <code>Button--justALittleSmaller</code>offline as well).
